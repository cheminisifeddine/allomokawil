...
const _worker = {
  'id': 16, 'user_id': 31, 'bio': 'دهان وديكور، خبرة طويلة في التشطيب الداخلي والخارجي',
...
const _project = {
  ...
  'customer_id': 30, 'title': 'دهان شقة 3 غرف', 'description': 'دهان كامل للشقة مع إصلاح الجدران المتضررة',
...
const _review = {
  ...
  'comment': 'عمل ممتاز ونظيف، أنصح به بشدة', 'images': '[]', 'is_visible': 1,
...
const _quote = {
  ...
  'message': 'جاهز للبدء فوراً، السعر يشمل المواد', 'estimated_days': 5,
  'status': 'pending',
  'created_at': '2026-09-11 20:23:45', 'updated_at': '2026-09-11 20:23:45',
...
const _message = {
  'id': 1, 'conversation_id': 5, 'sender_id': 31, 'content': 'مرحبا، متى يمكنك البدء؟',
  'type': 'text', 'image_url': null, 'is_read': 1,
...
Future<void> _loadFonts() async {
  ...
  stdout.writeln('FONTS: regular=${reg.lengthInBytes}b bold=${bold.lengthInBytes}b');
...
void main() {
  ...
  testWidgets('shots: customer', (tester) async {
    final s = await boot();
    await _shoot(tester, '04_customer_home', const CustomerHomeScreen(),
        s.api, s.auth);
    await _shoot(tester, '05_project_new', const ProjectNewScreen(),
        s.api, s.auth);
    await _shoot(tester, '06_projects',
        ProjectsScreen(repo: Repository(s.api)), s.api, s.auth);
    await _shoot(
        tester,
        '07_project_detail',
        ProjectDetailScreen(
  ...
  testWidgets('shots: worker', (tester) async {
    final s = await boot();
    await _shoot(tester, '08_worker_home', const WorkerHomeScreen(), s.api,
        s.auth);
  ...
  testWidgets('shots: browse + chat + misc', (tester) async {
    ...
    await _shoot(tester, '11_chat_list', ChatListScreen(repo: Repository(s.api)),
        s.api, s.auth);
    await _shoot(
        tester,
        '12_chat',
    ...
    await _shoot(tester, '14_verification', const VerificationScreen(),
        s.api, s.auth);
  });
}
